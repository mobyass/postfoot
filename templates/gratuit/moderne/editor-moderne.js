// editor-moderne.js - Script pour l'éditeur moderne
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function selectTeam(club) {
    if (currentTeam === "A") {
      ...
      recadrageLogoA.classList.remove('empty');
    } else {
      equipeB = club;
      updateTeam("B", club);
      logoB.style.backgroundImage = `url('${club.logo}')`;
      logoB.classList.remove('empty');
      recadrageLogoB.style.backgroundImage = `url('${club.logo}')`;
      recadrageLogoB.classList.remove('empty');
    }
  ...
  function openRecadrage() {
    ...
    // Synchroniser la couleur SVG
    const scoreSvgPath = document.getElementById("scoreSvgPath");
    const recadrageSvgPath = document.getElementById("recadrage-scoreSvgPath");
    if (scoreSvgPath && recadrageSvgPath) {
      recadrageSvgPath.setAttribute("fill", scoreSvgPath.getAttribute("fill"));
    }
  ...
  function exportImage() {
    html2canvas(capture, { scale: 4, useCORS: true }).then(canvas => {
      ...
      a.download = "post-match-moderne.png";
      a.click();
    });
  }
    ...
    html2canvas(capture, { scale: 4, useCORS: true }).then(canvas => {
      canvas.toBlob(blob => {
        if (navigator.share && navigator.canShare) {
          const file = new File([blob], "post-match-moderne.png", { type: "image/png" });
  ...
  // =====================
  // COULEUR PRINCIPALE (SVG + carré droit)
  // =====================
  const scoreSvgPath = document.getElementById("scoreSvgPath");
  const recadrageSvgPath = document.getElementById("recadrage-scoreSvgPath");
  const gradientColorPicker = document.getElementById("gradientColorPicker");

  // Calcule une couleur plus claire (×1.6) pour le carré de droite
  function lighterHex(hex) {
    const r = parseInt(hex.slice(1, 3), 16);
    const g = parseInt(hex.slice(3, 5), 16);
    const b = parseInt(hex.slice(5, 7), 16);
    return `rgb(${Math.min(255, Math.round(r * 1.6))}, ${Math.min(255, Math.round(g * 1.6))}, ${Math.min(255, Math.round(b * 1.6))})`;
  }

  // Charger la couleur sauvegardée au démarrage
  const savedColor = localStorage.getItem("moderneColor");
  if (savedColor) {
  ...
  gradientColorPicker.addEventListener("input", (e) => {
    ...
    applyColor(color);
    localStorage.setItem("moderneColor", color);
  });

  function applyColor(hex) {
    // Bande SVG courbe
    if (scoreSvgPath) scoreSvgPath.setAttribute("fill", hex);
    if (recadrageSvgPath) recadrageSvgPath.setAttribute("fill", hex);

    // Carré de droite (logoB) = version plus claire
    const lighter = lighterHex(hex);
    const logoBEl = document.getElementById("logoB");
    const recadrageLogoBEl = document.getElementById("recadrage-logoB");
    if (logoBEl) logoBEl.style.backgroundColor = lighter;
    if (recadrageLogoBEl) recadrageLogoBEl.style.backgroundColor = lighter;
  }
