...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function chargerClubFavori() {
    ...
    if (club) {
      ...
      if (logoA && club.logo) {
        const imgA = document.getElementById('logoA-img');
        if (imgA) imgA.src = club.logo;
        logoA.classList.add('has-logo');
        const clsA = document.getElementById('logoA-classique');
        if (clsA) { clsA.style.backgroundImage = `url('${club.logo}')`; clsA.classList.remove('empty'); }
      }
  ...
  function selectTeam(club) {
    if (currentTeam === "A") {
      ...
      if (imgA) imgA.src = club.logo;
      logoA.classList.add('has-logo');
      const recImgA = document.getElementById('recadrage-logoA-img');
      if (recImgA) recImgA.src = club.logo;
      recadrageLogoA.classList.add('has-logo');
      const clsA = document.getElementById('logoA-classique');
      if (clsA) { clsA.style.backgroundImage = `url('${club.logo}')`; clsA.classList.remove('empty'); }
    } else {
      equipeB = club;
      updateTeam("B", club);
      const imgB = document.getElementById('logoB-img');
      if (imgB) imgB.src = club.logo;
      logoB.classList.add('has-logo');
      const recImgB = document.getElementById('recadrage-logoB-img');
      if (recImgB) recImgB.src = club.logo;
      recadrageLogoB.classList.add('has-logo');
      const clsB = document.getElementById('logoB-classique');
      if (clsB) { clsB.style.backgroundImage = `url('${club.logo}')`; clsB.classList.remove('empty'); }
    }
  ...
  const savedColor = localStorage.getItem("bandColor");
  if (savedColor) {
    gradientColorPicker.value = savedColor;
    applyBandColor(savedColor);
  }

  // Gérer le changement de couleur
  gradientColorPicker.addEventListener("input", (e) => {
    const color = e.target.value;
    applyBandColor(color);
    localStorage.setItem("bandColor", color);
  });

  function applyBandColor(hexColor) {
    document.querySelectorAll(".mini-band").forEach(band => {
      band.style.background = hexColor;
    });
    document.querySelectorAll(".logo-classique-b").forEach(el => {
      el.style.backgroundColor = hexColor;
    });
  }

  // =====================
  // BAND INFO (équipes + compétition)
  // =====================
  function updateBandInfo() {
    const comp = info1 ? info1.textContent : "NATIONAL 1";
    const nomA = (equipeA ? equipeA.nom : "Équipe A").toUpperCase();
    const nomB = (equipeB ? equipeB.nom : "Équipe B").toUpperCase();
    [
      ["bandTeamA",           nomA + " VS"],
      ["bandTeamB",           nomB],
      ["bandComp",            comp],
      ["recadrage-bandTeamA", nomA + " VS"],
      ["recadrage-bandTeamB", nomB],
      ["recadrage-bandComp",  comp],
    ].forEach(([id, text]) => {
  ...
  btnSwapTeams.addEventListener("click", () => {
    ...
    const tempSrc = imgA ? imgA.src : '';
    if (imgA) imgA.src = imgB ? imgB.src : '';
    if (imgB) imgB.src = tempSrc;
    if (recImgA) recImgA.src = imgA ? imgA.src : '';
    if (recImgB) recImgB.src = imgB ? imgB.src : '';

    const tempHasLogoA = logoA.classList.contains('has-logo');
    logoA.classList.toggle('has-logo', logoB.classList.contains('has-logo'));
    logoB.classList.toggle('has-logo', tempHasLogoA);
    recadrageLogoA.classList.toggle('has-logo', logoA.classList.contains('has-logo'));
    recadrageLogoB.classList.toggle('has-logo', logoB.classList.contains('has-logo'));
